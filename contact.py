...
def find_contact(contacts):
    print("\n\nBuscar contacto\n")
    search_term = input("Introducir el nombre o mail del contacto o parte de él: ")
    found_contacts = []
    ...
    print("{} - {}".format(contact_counter, contact["name" or "email"]))
    ...
    if len(found_contacts) > 1:
        option = ask_until_option_expected(contact_indexes)
    elif len(contact_indexes) == 0:
        print("No se ha encontrado ninguno.")
        return

    print("\nInformacion sobre {}\n".format(found_contacts[contact_index]["name" or "email"]))
